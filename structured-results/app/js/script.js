...
function renderPagination(content) {
  ...
  for (page = content.page - 3; page < content.page + 3; ++page) {
    if (page < 0 || page >= content.nbPages) {
      continue;
    }
...
function setURLParams() {
  ...
  if (algoliaHelper.state.query.trim() !== '') {
    trackedParameters.push('query');
  }
  if (algoliaHelper.state.page !== 0) {
    trackedParameters.push('page');
  }
  if (algoliaHelper.state.index !== INDEX_NAME) {
    trackedParameters.push('index');
  }
